...
from bsery.domain import AccountData, BigSmallEtfRotate, StrategyAccount
from bsery.stockutil import StockUtil
...
class BigSmallEtfRotateStrategy:
    def __init__(self, account_id: int):
        ...
        if account is None:
            logger.error(
                f"invest.strategy_acount 账户{account_id} 初始状态为空，或者不正确，请立即修复!"
            )
        ...
        if not self.is_valid_status_cd(status=self.strategy_status_cd):
            raise Exception(
                f"stategy_status_cd 设置不正确, {self.strategy_status_cd}"
            )
        self.momentum_days = int(configs["momentum_days"].data)
        df_ticker = self.db.get_df_by_sql(
            f"select distinct ticker, sec_short_name from stock.fund where ticker='{self.ticker_x}' or ticker = '{self.ticker_y}'"
        )
        if df_ticker.shape[0] != 2:
            raise Exception(f"查询ticker 基金名字出错! 查询结果为: {df_ticker}")
        self.ticker_x_name = df_ticker.loc[
            df_ticker["ticker"] == self.ticker_x, "sec_short_name"
        ].iloc[0]
    ...
    def get_strategy_by_date(
        ...
        mom_begin_date = self.util.get_trade_day_by_offset(
            start_date, self.momentum_days + 1
        )
        ...
        df_load = df_load[df_load["trade_date"] >= mom_begin_date]
        ...
        df["big_mom"] = df["close_price_x"].pct_change(
            periods=self.momentum_days
        )
        df["small_mom"] = df["close_price_y"].pct_change(
            periods=self.momentum_days
        )
        ...
        df.loc[
            ((df["pre_pos"] != df["pos"]) & (df["pos"] != "empty")), "trade_cd"
        ] = 3

        df.loc[
            ((df["pre_pos"] == "empty")) & (df["pos"] != "empty"), "trade_cd"
        ] = 1

        df.loc[
            ((df["pre_pos"] == "big") | (df["pre_pos"] == "small"))
            & (df["pos"] == "empty"),
            "trade_cd",
        ] = 2
        ...
        # 买入收益调整
        df.loc[
            (df["trade_cd"] == 1) & (df["pos"] == "big"),
            "strategy_chg_pct_adj",
        ] = (df["close_price_x"] / df["open_price_x"]) - (1 + self.trade_rate)
        df.loc[
            (df["trade_cd"] == 1) & (df["pos"] == "small"),
            "strategy_chg_pct_adj",
        ] = (df["close_price_y"] / df["open_price_y"]) - (1 + self.trade_rate)

        # 卖出收益调整
        df.loc[
            (df["trade_cd"] == 2) & (df["pre_pos"] == "big"),
            "strategy_chg_pct_adj",
        ] = (
            df["open_price_x"]
            * (1 - self.trade_rate)
            / df["pre_close_price_x"]
        ) - 1
        df.loc[
            (df["trade_cd"] == 2) & (df["pre_pos"] == "small"),
            "strategy_chg_pct_adj",
        ] = (
            df["open_price_y"]
            * (1 - self.trade_rate)
            / df["pre_close_price_y"]
        ) - 1

        # 换仓收益调整, 减两次手续费
        df.loc[
            (df["trade_cd"] == 3) & (df["pos"] == "big"),
            "strategy_chg_pct_adj",
        ] = (
            (
                df["open_price_y"]
            ...
            - 1
            + (df["close_price_x"] / df["open_price_x"])
            - (1 + self.trade_rate)
        )
        df.loc[
            (df["trade_cd"] == 3) & (df["pos"] == "small"),
            "strategy_chg_pct_adj",
        ] = (
            (
                df["open_price_x"]
    ...
    def calc_df_with_account_data(
        self, init_account_data: AccountData, df: pd.DataFrame
    ) -> pd.DataFrame:
        """第一行数据为初始化数据
        ...
        for index, row in df.iterrows():
            try:
                ...
                raise Exception(f"df 状态不对, Exception: {e} 状态保存至:{debug_file}")
    ...
    def write_df_to_db(self, df: pd.DataFrame) -> int:
        ...
        if self.strategy_status_cd == "S":
            logger.info(
                f"{self.account.account_id} suspended，skip writing data to db!"
            )
            return 0

        account_id = self.account.account_id
        df_db = self.db.get_df_by_sql(
            f"select * from invest.big_small_etf_rotate where account_id = {account_id}"
        )
        ...
        for index, row in df.iterrows():
            ...
            if self.strategy_status_cd == "P":
                ...
                bser.strategy_net_real = pre.net
            else:
                ...
                bser.strategy_chg_pct_real = row[
                    "strategy_chg_pct_real"
                ]  # 实际交易中扣除手续费后的涨幅
    ...
    def calc_new_account_data(
        self, pre: AccountData, row: pd.Series
    ) -> AccountData:
        """计算交易多少股票,以及手续费
        Args:
            pre: 前一日的股票账户信息
        ...
        # 买入
            ...
            cur.vol = (
                int(pre.balance / (to_buy_price * (1 + self.trade_rate) * 100))
                * 100
            )
            ...
            if pre_pos_style == "big":
                ...
                raise Exception(
                    f"前日持仓数据有问题: {pre_pos_style} pre:{pre}, cur:{cur}"
                )

            cur.balance = (
                pre.vol * to_sale_price * (1 - self.trade_rate) + pre.balance
            )
        ...
        # 换仓
            ...
            # 先卖后买
            # 卖出操作之后
            cur.balance = pre.balance + pre.vol * to_sale_price * (
                1 - self.trade_rate
            )
            sale_trade_fee = pre.vol * to_sale_price * self.trade_rate
            # 然后买入, 以开盘价买入是存在误差的，这里忽略这种误差
            cur.vol = (
                int(cur.balance / (to_buy_price * (1 + self.trade_rate) * 100))
                * 100
            )
    ...
    def backtest_by_range(
        self, date_list: list, show_plot: bool
    ) -> pd.DataFrame:
        pass
    ...
    def run_with_plot(self, start_date: datetime):
        ...
        for _start_date in _backtest_sec:
            ...
            df = self.get_strategy_by_date(_start_date)
